...
export default function Home() {
  ...
  try {
    connectServer("user/login", fakeData).then((res) => {
      if (res.status == 202) {
  ...
  function signIn() {
    ...
    connectServer("user/login", loginObj).then((res) => {
      if (res.status === 200) {
        //status 200 property
        stateAction(
          "success",
  ...
  return (
    <>
      <div className={"container"}>
        ...
        />
        <Input
          el={<img src="../person.svg" alt="personIcon" />}
          className={"input"}
          ref={userName}
          placeholder={"Enter your username"}
        />
        <Input
          el={<img src="../key.svg" alt="keyIcon" />}
          className={"input"}
          ref={password}
          type="password"
        ...
        <UrlText
          className={"link"}
          href={{ pathname: "./pages/resetPassword" }}
          text={""}
          urlText={"Forgot password?"}
        />
      </div>
      <UrlText
        href={{ pathname: "./pages/createAccount" }}
        className={"signup"}
