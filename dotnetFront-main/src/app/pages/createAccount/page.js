...
export default function CreateAccount() {
    ...
    function verify() {
        if (!userName.current.value.isValidName()) {
            ...
            stateAction("success", undefined, userName.current);
        }
        if (!email.current.value.isEmail()) {
            stateAction("faild", undefined, email.current);
        } else
            stateAction("success", undefined, email.current);

        if (card.current.value.length < 11) {
            stateAction("faild", undefined, card.current);
        }
        else
            stateAction("success", undefined, card.current);
        if (password.current.value.length < 8) {
            stateAction("faild", undefined, password.current);
        }
        else
            stateAction("success", undefined, password.current);
    ...
    return (
        <div className="container c2">
            ...
            <Input el={<img src="../mail.svg" alt="test" />} className="input" placeholder="Email" type="email" ref={email} key={"username"} />
            <Input el={<img src="../card.svg" alt="test" />} className="input" type="number" placeholder="Credit card" ref={card} key={"username"} />
